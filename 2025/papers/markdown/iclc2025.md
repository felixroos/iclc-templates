...
The purpose of the runtime is to create an environment where the compiled code can be executed. To be useful as a live coding system, this runtime should also handle code updates, hot-reloading and real-time audio/video feedback.
An example implementation of this runtime can be found in the Web Audio version of KabelSalat. The runtime is located in an AudioWorklet that communicates with the rest of the application via a MessagePort [@Roberts18].
...
KabelSalat was mainly used for live looping a trumpet and the input from a MIDI keyboard.
Inspired by tape loops, a looper node has been created using long delay lines.
