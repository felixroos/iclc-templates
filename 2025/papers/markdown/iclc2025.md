...
![KabelSalat web interface ([https://kabel.salat.dev](https://kabel.salat.dev), accessed on September 27, 2024) running a looper patch. On the left pane pane: source code editor with in-source widgets. On the right pane: the real-time audio graph visualizer.](images/repl2.png)
...
KabelSalat is based on a terse syntax that relies heavily on method chaining. It is directly inspired by the syntax of Hydra [@hydra] and Strudel [@strudel]^[Strudel used the same technique applied to a different domain: the functional composition of musical patterns. Hydra uses it, similarly to KabelSalat, as a way to connect and combine video processing nodes.], providing some degree of familiarity for new users already familiar with these tools.
In KabelSalat's context, chaining methods can be seen as a way to emulate the patch point connections between the different modules of a synthesizer. Both functions and methods can be considered as signal generators or processors (nodes), which can be connected to other modules through chaining or reference. Arguments of these nodes can either be constant values or other nodes. The same basic principles can be used for creating patches of arbitrary depth and complexity. The following code example shows how a classic subtractive synthesizer patch can be written in KabelSalat. Figure 2 is the visual representation of the same audio graph, which has been automatically generated in the REPL window.
...
Feedback is also an important creative technique in the domain of live visuals^[See for instance: [https://andreijaycreativecoding.com/getting_started-with-video-feedback](https://andreijaycreativecoding.com/getting_started-with-video-feedback) (accessed September 28, 2024).
...
The generated code expects some variables to be in scope to run properly: `nodes` (instances of stateful nodes), `r` (node value registers) and `o` (output channel registers). These variables are used in the example above. The following sub-sections will examine each of these variables in further detail.
...
```js
let impulse = registerNode("impulse", {
  ugen: "ImpulseOsc",
...
Graph compilation is generic enough to be used in different kinds of real-time audio/video applications central to live coding. So far, in order to test our method, we have developed a WebAudio version, a GLSL shader version mimicking Hydra (_hydro_) and a version that compiles to static C code.
...
The KabelSalat REPL^[Website link: [https://kabel.salat.dev/](https://kabel.salat.dev/) (accessed on September 27, 2024). ] uses the latest version of KabelSalat's web version.
It can be used as a way to experiment, share patches and live code without any audio interruption.
It consists of a code editor, a graph visualizer, example patches, and an interactive documentation.
The URL always reflects the latest code change, allowing patches to be shared as a hyperlink.

The flok^[Flok is a peer-to-peer collaborative live coding environment created by Damián Silvani. The website is available under [https://flok.cc/](https://flok.cc/) (accessed on September 27, 2024).]-based, collaborative live coding site nudel^[Website link: [https://nudel.cc/](https://nudel.cc/) (accessed on April 6, 2025). ] now includes KabelSalat as a language, along with experimental bindings to Strudel patterns.

To test KabelSalat as an audio backend for a custom UI, BlockSalat^[Website link: [https://block.salat.dev/](https://block.salat.dev)] was created. It uses the block based visual programming tool blockly [@blockly] to connect KabelSalat nodes.

## Real Time Input

The KabelSalat REPL supports both Audio and MIDI Input (through the Web MIDI API).
These inputs allow direct integration with the code through a microphone, line input or through MIDI Controllers.
As a result, it can be used as a synthesis-oriented companion tool for various live coding setups, allowing the live coding of synthesizers and audio treatments on-the-fly. Because AudioWorklet's process audio in blocks of 128 samples, the latency is low enough to play synths with a midi keyboard and/or record audio input in sync.

Similar to the Strudel REPL [@strudel], the code editor supports in-source UI elements, such as buttons and sliders. Each UI element has a corresponding function name, e.g. `_` for slider and `B` for a button, which will be transformed in-place into an interactive HTML element on evaluation. Similar to regular text, these elements can be moved, deleted, copied and pasted from within the code. Compared to entering numbers directly, they offer a more direct gestural manipulation of values, tightening the feedback loop between performer and program. Instead of re-evaluating the code, changes in UI elements are communicated via the MessagePort to the AudioWorklet. To make sure there is no disjoint between the state of the UI and the state of the code, each interaction with a UI element also changes the code itself, persisting any state that might be volatile.
...
Additionally, jan Ten has used KabelSalat as a target for ORCA sequences, using both tools side-by-side. While ORCA sequences were live coded, KabelSalat was mainly used to set up instruments and change parameters via sliders^[jan Ten's performance on Youtube: [https://www.youtube.com/watch?v=wiHH35GR908](https://www.youtube.com/watch?v=wiHH35GR908) (_idem_)].
...
Though KabelSalat and its surrounding ecosystem show promising potential, its original goal of augmenting SuperDough has not yet been reached.
The current implementation comes with a set of limitations that need to be adressed in future research.
Only the most glaring ones will be addressed briefly in this section.

- **Spawning `Units`**: While it is possible to have two `Unit` instances running side-by-side for the purpose of cross-fading, we still cannot schedule the creation and destruction of `Unit`'s in a more flexible or creative way. Such a mechanism is required to queue events similar to SuperDough in Strudel [@strudel], or to build more complex audio patches. In the future, further steps will be taken in the direction of becoming an event based audio engine, as required by SuperDough.^[A similar mechanism is available in SuperCollider, as explained in (https://docs.supercollider.online/Guides/Spawning.html) (accessed on April 6, 2025).]

- **Preservation of state**: As discussed in section 4.2, the current version does not reuse nodes from previous evaluations, causing any state to reset on each update. This behavior hinders the perception of liveness for the musician using KabelSalat for live coding.

- **Performance**: So far, we have not conducted reproducible performance tests. Potential performance gains could be achieved in the web version by compiling to WebAssembly instead of JavaScript [@Roberts22].

- **Synergy with Tidal patterns**: Tidal patterns [@tidal;@tidal2] might be combined with an audio graph in a specific way, using Patterns as inputs for individual nodes, rather than composing expressions to a single pattern. Inversely, patterns could also construct a graph for each event, instead of creating a shallow key-value-mapping to set parameters.

- **Standard library improvements**: more complex, versatile and practical UGens should be added to accommodate typical computer music needs, avoiding boilerplate code and facilitating rapid experimentation and sound design.
