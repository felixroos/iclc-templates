...
Similar to the Strudel REPL [@strudel], the code editor supports in-source UI elements, such as buttons and sliders. Each UI element has a corresponding function name, e.g. `_` for slider and `B` for checkbox, that will be transformed into an interactive HTML element when evaluated. In contrast to other mixed code and UI applications like Faust, these elements are part of the code itself, as they will move with the text
