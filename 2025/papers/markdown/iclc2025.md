...
The Kabelsalat REPL is available under `kabel.salat.dev`. It consists of a code editor, a graph visualizer, example patches and an interactive documentation. Similar to the Strudel REPL [@strudel], the code editor supports in-source UI elements, such as buttons and sliders. The URL always reflects the latest code change, allowing patches to be shared as a hyperlink.
