...
The project is still in an early phase and comes with a set of limitations. While it is possible to have two `Unit` instances running side-by-side for the purpose of cross-fading, we still cannot schedule the creation and destruction of `Unit`'s in a more flexible way. Such a mechanism is required to queue events similar to SuperDough in Strudel [@strudel].
As discussed in chapter 4.2, the current version does not reuse nodes from previous evaluations, causing any state to reset on each update.
Furthermore, potential performance gains could be achieved in the web version by compiling to WebAssembly instead of JavaScript [@Roberts22].
...
Inversely, patterns could also construct a graph for each event, instead of creating a shallow key-value-mapping to set parameters.
...
Thanks to Maxime Chevalier-Boisvert for creating NoiseCraft, which was the starting point of the project.
Thanks to pulu for being an early adopter, creating mesmerizing patches and providing valuable feedback and contributions. Thanks to Raphaël Maurice Forment for being a good conversation partner during the journey of the implementation of KabelSalat, eventually joining this paper as a second author, helping with proof-reading, phrasing and further research.
